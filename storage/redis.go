...
func (r *RedisClient) WriteShare(login, id string, params []string, diff int64, height uint64, window time.Duration, hostname string) (bool, error) {
	...
	_, err = tx.Exec(func() error {
		r.writeShare(tx, ms, ts, login, id, diff, window, hostname, r.pplns)
...
func (r *RedisClient) WriteBlockSolo(login, id string, params []string, diff, roundDiff int64, height uint64, window time.Duration, hostname string) (bool, error) {
	exist, err := r.checkPoWExist(height, params)
	if err != nil {
		return false, err
	}
...
func (r *RedisClient) writeShare(tx *redis.Multi, ms, ts int64, login, id string, diff int64, expire time.Duration, hostname string, pplns int64) {
	/* # Note To Me:
