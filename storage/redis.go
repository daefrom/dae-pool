...
func (b *BlockData) key() string {
	return join(b.UncleHeight, b.Orphan, b.Nonce, b.serializeHash(), b.Timestamp, b.Difficulty, b.TotalShares, b.Finder, b.Reward, b.Worker)
}
...
func (r *RedisClient) CollectWorkersStats(sWindow, lWindow time.Duration, login string) (map[string]interface{}, error) {
	...
	for id, worker := range workers {
		...
		log.Println("Lats beat: %s , widnow: %s", worker.LastBeat, now-smallWindow/2)
		if worker.LastBeat < (now - smallWindow/2) {
			worker.Offline = true
			offline++
			ret := r.GetWorker(login, id)
			log.Println("WORKER: %s", ret)
			if ret == "0" {
				...
				if result != "NA" {
					log.Println("WORKER EMAIL: %s", result)
					send(body, result)
...
func send(body string, to string) {
	...
	bodyU := body + " " + formatted
	msg := composeMimeMail(to, from, subject, bodyU)
...
func formatEmailAddress(addr string) string {
	from := mail.Address{"DAEPool", addr}
